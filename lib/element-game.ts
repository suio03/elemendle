...
export function compareElements(guess: Element, target: Element): ElementGuessResult {
  ...
  return {
    ...
    matches: {
      ...
      elementType: guess.classic.element_type === target.classic.element_type,
      phase: guess.classic["phase-at-stp"] === target.classic["phase-at-stp"],
...
export function getDailyElement(elements: Record<string, Element>): Element {
  const elementsList = Object.values(elements);
  const today = new Date();
  const seed = today.getFullYear() * 10000 + (today.getMonth() + 1) * 100 + today.getDate();
  const index = seed % elementsList.length;
